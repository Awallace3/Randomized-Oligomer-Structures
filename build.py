    ...
    for i in range(len(geom[:,0])):
        for j in range(len(geom[:,0])):
            ...
            if j > i and distances < 1.8 :
                ...
                line = str(i + 1) + " ", str(j + 1) + " ", "=" + str(distances), " B", "\n", str(i + 1) + " ", str(j + 1) + " ", "F\n"
...
def constraints(molecule_cnt, spacer, dihedral=False):
    ...
    if dihedral == True:
        bsas = pd.read_csv('dihedral.txt', sep=' ', header=None)
    ...
    for i in range(num):
        ...
        if dihedral == True: 
            for row in dh_ang.itertuples():
                if dh_ang.columns[0]==int:
                    dh_ang.at[row.Index, 0] += spacer
                if dh_ang.columns[1]==int:
                    dh_ang.at[row.Index, 1] += spacer
                if dh_ang.columns[2]==int:
                    dh_ang.at[row.Index, 2] += spacer
                if dh_ang.columns[3]==int:
                    dh_ang.at[row.Index, 3] += spacer
...
for i in range(1, 6, 1):
    ...
    final, mole, spacer = random_arrangement(geo, 8)
